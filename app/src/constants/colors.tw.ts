export const colors = {
  primary: {
    text: "text-sky-600 dark:text-sky-500",
    accent: "accent-sky-600 dark:accent-sky-500",
    bg: "bg-sky-500",
    border: "border-sky-500",
  },
  secondary: {
    text: "text-pink-500 dark:text-pink-400",
    accent: "accent-pink-500 dark:accent-pink-400",
    bg: "bg-pink-400",
    border: "border-pink-500",
  },
  white: {
    text: "text-zinc-50 dark:text-neutral-800",
    accent: "accent-zinc-50 dark:accent-neutral-800",
    bg: "bg-white dark:bg-neutral-900",
  },
  black: {
    text: "text-zinc-800 dark:text-neutral-200",
    accent: "accent-zinc-800 dark:accent-neutral-200",
    bg: "bg-black dark:bg-neutral-300",
  },
  success: {
    text: "text-emerald-600",
    accent: "accent-emerald-600",
    bg: "bg-emerald-600",
  },
  warning: {
    text: "text-orange-500",
    accent: "accent-orange-500",
    bg: "bg-orange-500",
  },
  error: {
    text: "text-red-500",
    accent: "accent-red-500",
    bg: "bg-red-500",
  },

  gray0: {
    text: "text-gray-50 dark:text-gray-950",
    accent: "accent-gray-50 dark:accent-gray-950",
    bg: "bg-gray-50 dark:bg-gray-950",
    border: "border-gray-50 dark:border-gray-950",
  },
  gray1: {
    text: "text-gray-100 dark:text-gray-900",
    accent: "accent-gray-100 dark:accent-gray-900",
    bg: "bg-gray-100 dark:bg-gray-900",
    border: "border-gray-100 dark:border-gray-900",
  },
  gray2: {
    text: "text-gray-200 dark:text-gray-800",
    accent: "accent-gray-200 dark:accent-gray-800",
    bg: "bg-gray-200 dark:bg-gray-800",
    border: "border-gray-200 dark:border-gray-800",
  },
  gray3: {
    text: "text-gray-300 dark:text-gray-700",
    accent: "accent-gray-300 dark:accent-gray-700",
    bg: "bg-gray-300 dark:bg-gray-700",
    border: "border-gray-300 dark:border-gray-700",
  },
  gray4: {
    text: "text-gray-400 dark:text-gray-600",
    accent: "accent-gray-400 dark:accent-gray-600",
    bg: "bg-gray-400 dark:bg-gray-600",
    border: "border-gray-400 dark:border-gray-600",
  },
  gray5: {
    text: "text-gray-500",
    accent: "accent-gray-500",
    bg: "bg-gray-500",
    border: "border-gray-500",
  },
  gray6: {
    text: "text-gray-600 dark:text-gray-400",
    accent: "accent-gray-600 dark:accent-gray-400",
    bg: "bg-gray-600 dark:bg-gray-400",
    border: "border-gray-600 dark:border-gray-400",
  },
  gray7: {
    text: "text-gray-700 dark:text-gray-300",
    accent: "accent-gray-700 dark:accent-gray-300",
    bg: "bg-gray-700 dark:bg-gray-300",
    border: "border-gray-700 dark:border-gray-300",
  },
  gray8: {
    text: "text-gray-800 dark:text-gray-200",
    accent: "accent-gray-800 dark:accent-gray-200",
    bg: "bg-gray-800 dark:bg-gray-200",
    border: "border-gray-800 dark:border-gray-200",
  },
  gray9: {
    text: "text-gray-900 dark:text-gray-100",
    accent: "accent-gray-900 dark:accent-gray-100",
    bg: "bg-gray-900 dark:bg-gray-100",
    border: "border-gray-900 dark:border-gray-100",
  },
} as const;

export const RouteBackgroundColor = "bg-zinc-800";
