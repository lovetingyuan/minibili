  ...
  return (
    <View className="w-full">
      <View className="relative w-full">
        ...
        {watchedInfo ? (
          <View
            className={`absolute bottom-0 left-0 h-1.5 ${colors.secondary.bg}`}
      ...
      <Text
        className={clsx('mt-3 min-h-8', isFollowed && ['font-bold', colors.primary.text])}
        numberOfLines={2}
      >
        {video.title}
      </Text>
      <View className="mt-2 flex-row items-center justify-between">
        <View className="shrink flex-row items-center">
          {isFollowed ? (
            ...
            <Image className="h-[11px] w-[13px]" source={require('../../../assets/up-mark.png')} />
          )}
          <Text
            numberOfLines={1}
            ellipsizeMode="tail"
