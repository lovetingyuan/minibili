...
function assignSnapshotValue<Snapshot extends Record<string, unknown>, Key extends keyof Snapshot>(
  target: Partial<Snapshot>,
  key: Key,
  value: Snapshot[Key],
) {
  target[key] = value;
...
export function resolveInitialSync<Key extends string, Snapshot extends Record<Key, unknown>>(
  ...
  const hasRemoteData = keys.some((key) =>
    Object.prototype.hasOwnProperty.call(remoteSnapshot, key),
  );
  if (!hasRemoteData) {
    return {
      hasRemoteData: false,
