...
export function Additional(props: { additional?: AdditionalType | null }) {
  ...
  if (additional.type === HandledAdditionalTypeEnum.ADDITIONAL_TYPE_RESERVE) {
    const desc = [additional.reserve.desc1?.text, additional.reserve.desc2?.text]
      .filter(Boolean)
      .join("  ");
  ...
  } else if (additional.type === HandledAdditionalTypeEnum.ADDITIONAL_TYPE_UGC) {
    const text = [additional.ugc.head_text, additional.ugc.desc_second, additional.ugc.duration]
      .filter(Boolean)
      .join("  ");
  ...
  } else if (additional.type === HandledAdditionalTypeEnum.ADDITIONAL_TYPE_COMMON) {
    const text = [additional.common.head_text, additional.common.desc1, additional.common.desc2]
      .filter(Boolean)
      .join("  ");
  ...
  } else if (additional.type === HandledAdditionalTypeEnum.ADDITIONAL_TYPE_GOODS) {
    const text = [additional.goods.items[0]?.name, additional.goods.items[0]?.price]
      .filter(Boolean)
      .join(" ");
  ...
  } else if (additional.type === HandledAdditionalTypeEnum.ADDITIONAL_TYPE_VOTE) {
  ...
  } else if (additional.type === HandledAdditionalTypeEnum.ADDITIONAL_TYPE_MATCH) {
    url = parseUrl(additional.match.jump_url);
    const { title, center_top, center_bottom, left_team, right_team } = additional.match.match_info;
    additionalContent = (
      <View className="gap-2">
        <Text>赛事：{additional.match.head_text}</Text>
        <Text>
          {title} {left_team.name} VS {right_team.name} {`${center_bottom}-${center_top.join("")}`}
        </Text>
      </View>
    );
  } else if (additional.type === HandledAdditionalTypeEnum.ADDITIONAL_TYPE_UPOWER_LOTTERY) {
  ...
  const linkProp = url
    ...
    : {};
  return (
    <Foo {...linkProp} className={`mt-5 flex-1 border-l py-1 pl-2 ${colors.gray6.border}`}>
      {additionalContent}
    </Foo>
  );
}
