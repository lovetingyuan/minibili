...
import { createOtpCode, OTP_EXPIRY_MS, OTP_RESEND_COOLDOWN_MS, OTP_RETRY_LIMIT } from '../../user-record-store'
...
async function handleAuthStatus(c: AppContext) {
  const body = await readJsonBody(c)
  const email = typeof body?.email === 'string' ? normalizeEmail(body.email) : null
  const token = typeof body?.token === 'string' && TOKEN_PATTERN.test(body.token) ? body.token : null
  if (!email || !token) {
