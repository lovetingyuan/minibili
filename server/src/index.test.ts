...
describe('server routes', () => {
  test('health returns service status payload', async () => {
    const app = createApp()
    const response = await app.fetch(new Request('https://example.com/health'), createEnv() as ServerBindings)
  ...
  test('static asset requests fall back to assets binding', async () => {
    ...
    const response = await app.fetch(new Request('https://example.com/styles.css'), env as ServerBindings)
  ...
  test('unknown api routes still return 404 instead of hitting assets', async () => {
    ...
    const response = await app.fetch(new Request('https://example.com/api/unknown'), env as ServerBindings)
